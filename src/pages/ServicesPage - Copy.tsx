...
import { Building2, Clock3, Loader2, MapPin, Search, SlidersHorizontal, User, X } from 'lucide-react';
  ...
  return (
    <div className="bg-white">
      ...
      <section id="services-grid" className="px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-8 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <h2 className="text-3xl font-bold text-[#1A1A2E]">{hasSearched ? ui.title : ui.hint}</h2>
            {hasSearched ? <p className="text-sm text-[#6B7280]">{sortedResults.length} {ui.results}{appliedCity ? ` • ${appliedCity}` : ''}</p> : null}
          </div>

          {loading ? (
            <div className="rounded-2xl border border-[#E5E7EB] bg-white p-10 text-center"><Loader2 className="mx-auto animate-spin text-[#4FC3F7]" size={28} /></div>
          ) : !hasSearched ? (
            <div className="rounded-2xl border border-dashed border-[#D1E7F7] bg-[#F8FCFF] px-5 py-12 text-center text-sm font-medium text-[#4B5563]">{ui.hint}</div>
          ) : sortedResults.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-[#D1E7F7] bg-[#F8FCFF] px-5 py-12 text-center text-sm font-medium text-[#4B5563]">{ui.noResult}</div>
          ) : (
            <>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {paginatedResults.map((cleaner) => {
                  ...
                  return (
                    <article key={cleaner.id} className="group flex flex-col rounded-3xl border border-[#EEF2F7] bg-white shadow-[0_4px_24px_rgba(17,24,39,0.06)] transition-shadow hover:shadow-[0_8px_32px_rgba(17,24,39,0.1)]">
                      <div className="flex flex-col gap-3 p-5 sm:p-6">
                        <div className="flex items-start gap-3.5">
                          <div className="relative flex-shrink-0">
                            {cleaner.photoUrl ? <img src={cleaner.photoUrl} alt={cleaner.displayName} className="h-14 w-14 rounded-2xl object-cover" /> : <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[rgba(79,195,247,0.2)] to-[rgba(168,230,207,0.2)] text-[#4FC3F7]"><User size={22} /></div>}
                            <span className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full border-2 border-white bg-[#4ADE80]" />
                          </div>
                          <div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><p className="truncate text-base font-bold text-[#1A1A2E]">{cleaner.displayName}</p><span className="flex-shrink-0 rounded-xl bg-[rgba(79,195,247,0.12)] px-2.5 py-1 text-sm font-bold text-[#0284C7]">{formatRate(cleaner.hourlyRate)}</span></div></div>
                        </div>
                        {tags.length > 0 ? <div className="flex flex-wrap items-center gap-1.5">{tags.map((service) => <span key={`${cleaner.id}-${service}`} className="rounded-lg bg-[#F8FAFC] px-2.5 py-1 text-[11px] font-semibold text-[#4B5563]">{SERVICE_LABELS[service][language]}</span>)}{cleaner.services.length > 3 ? <span className="text-[11px] text-[#9CA3AF]">+{cleaner.services.length - 3}</span> : null}</div> : null}
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="inline-flex items-center gap-1 rounded-lg bg-[rgba(168,230,207,0.3)] px-2.5 py-1 text-[11px] font-semibold text-[#065F46]"><MapPin size={10} />Zone</span>
                          <span className="inline-flex items-center gap-1 rounded-lg bg-[rgba(79,195,247,0.12)] px-2.5 py-1 text-[11px] font-semibold text-[#0284C7]"><Clock3 size={10} />{ui.avail}</span>
                        </div>
                      </div>
                      <div className="mx-5 border-t border-[#F0F4F8] sm:mx-6" />
                      <div className="flex items-center gap-2.5 p-4 sm:p-5">
                        <button type="button" onClick={() => setModalCleaner(cleaner)} className="flex-1 rounded-2xl border border-[#E5E7EB] px-4 py-2.5 text-sm font-semibold text-[#4B5563] transition-colors hover:border-[#4FC3F7] hover:text-[#0284C7]">{ui.details}</button>
                        <button type="button" onClick={() => setAvailabilityCleaner(cleaner)} className="flex-1 rounded-2xl bg-[#4FC3F7] px-4 py-2.5 text-sm font-bold text-white shadow-[0_4px_12px_rgba(79,195,247,0.35)] transition-all hover:bg-[#38B2E8]">{ui.avail}</button>
                      </div>
                    </article>
                  );
                })}
              </div>
              <PaginationControls page={resultsPage} totalPages={totalPages} onPageChange={setResultsPage} labels={{ previous: ui.previous, next: ui.next, page: ui.page }} />
            </>
          )}
        </div>
      </section>
      ...
      <section className="bg-[#F8FCFF] px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <div className="max-w-3xl">
            ...
            <p className="mt-2 text-sm text-[#4B5563] sm:text-base">{ui.pricingSubtitle}</p>
          </div>
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {PRICING_BLOCKS[language].map((item) => (
              <article key={item.title} className="rounded-2xl border border-[#D1E7F7] bg-white p-5 shadow-[0_6px_24px_rgba(17,24,39,0.06)]">
      ...
      <section className="px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-8 grid gap-3">
            {FAQ_BY_LANG[language].map((item) => (
              <details key={item.q} className="group rounded-2xl border border-[#E5E7EB] bg-white p-4 open:border-[#D1E7F7] open:bg-[#F8FCFF]">
                <summary className="cursor-pointer list-none text-sm font-semibold text-[#1A1A2E]">
                  {item.q}
                </summary>
                <p className="mt-2 text-sm leading-relaxed text-[#4B5563]">{item.a}</p>
              </details>
            ))}
          </div>
        </div>
      ...
      <section className="px-4 pb-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl rounded-3xl border border-[#D1E7F7] bg-[linear-gradient(135deg,rgba(79,195,247,0.12),rgba(168,230,207,0.2))] p-6 sm:p-8">
          <h2 className="text-xl font-bold text-[#1A1A2E] sm:text-2xl">{ui.marketplaceTitle}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-relaxed text-[#374151] sm:text-base">{ui.marketplaceSubtitle}</p>
        </div>
      </section>

      {modalCleaner ? (
        <div className="fixed inset-0 z-[70] flex items-center justify-center overflow-y-auto overscroll-contain bg-black/40 p-4" onClick={() => setModalCleaner(null)}>
          <div className="flex max-h-[calc(100dvh-2rem)] w-full max-w-lg flex-col overflow-hidden rounded-3xl bg-white shadow-[0_20px_60px_rgba(17,24,39,0.25)]" onClick={(event) => event.stopPropagation()}>
            <div className="flex flex-shrink-0 items-start gap-4 p-6 pb-5">
              {modalCleaner.photoUrl ? <img src={modalCleaner.photoUrl} alt={modalCleaner.displayName} className="h-16 w-16 flex-shrink-0 rounded-2xl object-cover" /> : <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-[rgba(79,195,247,0.2)] to-[rgba(168,230,207,0.2)] text-[#4FC3F7]"><User size={24} /></div>}
              <div className="min-w-0 flex-1"><p className="text-lg font-bold text-[#1A1A2E]">{modalCleaner.displayName}</p><p className="mt-0.5 text-sm font-semibold text-[#0284C7]">{formatRate(modalCleaner.hourlyRate)}</p></div>
              <button type="button" onClick={() => setModalCleaner(null)} className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full border border-[#E5E7EB] text-[#9CA3AF] transition-colors hover:border-[#DC2626] hover:text-[#DC2626]"><X size={16} /></button>
            </div>
            <div className="border-t border-[#F0F4F8]" />
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-6"><div className="rounded-2xl bg-[#F8FAFC] p-4"><p className="mb-2 text-[11px] font-bold uppercase tracking-[0.12em] text-[#9CA3AF]">Description</p><p className="whitespace-pre-wrap text-sm leading-relaxed text-[#4B5563]">{modalCleaner.description || '--'}</p></div></div>
            <div className="flex flex-shrink-0 items-center gap-3 border-t border-[#F0F4F8] p-5">
              <button type="button" onClick={() => setModalCleaner(null)} className="flex-1 rounded-2xl border border-[#E5E7EB] py-2.5 text-sm font-semibold text-[#6B7280] transition-colors hover:border-[#9CA3AF]">{ui.close}</button>
              <button type="button" onClick={() => { setModalCleaner(null); setAvailabilityCleaner(modalCleaner); }} className="flex flex-1 items-center justify-center rounded-2xl bg-[#4FC3F7] py-2.5 text-sm font-bold text-white shadow-[0_4px_12px_rgba(79,195,247,0.35)] transition-all hover:bg-[#38B2E8]">{ui.avail}</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {availabilityCleaner ? (
        <div className="fixed inset-0 z-[75] flex items-center justify-center overflow-y-auto overscroll-contain bg-black/45 p-4" onClick={() => setAvailabilityCleaner(null)}>
          <div className="w-full max-w-2xl max-h-[calc(100dvh-2rem)] overflow-y-auto overscroll-contain rounded-3xl bg-white p-6 shadow-[0_20px_60px_rgba(17,24,39,0.28)]" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-3">
              <div><h3 className="text-base font-bold text-[#1A1A2E]">{ui.avail}</h3><p className="mt-0.5 text-sm font-semibold text-[#0284C7]">{availabilityCleaner.displayName}</p></div>
              <button type="button" onClick={() => setAvailabilityCleaner(null)} className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-[#E5E7EB] text-[#9CA3AF] transition-colors hover:border-[#DC2626] hover:text-[#DC2626]"><X size={16} /></button>
            </div>
            <div className="mt-4 rounded-2xl border border-[#E5E7EB] bg-[#F8FAFC] p-4">
              {availabilityRows.length === 0 ? <p className="text-sm text-[#6B7280]">{ui.noAvailability}</p> : <div className="space-y-2">{availabilityRows.map((row) => <div key={row.day} className="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-sm"><span className="font-semibold text-[#1A1A2E]">{dayLabel(row.day)}</span><span className="text-[#4B5563]">{`${row.start} - ${row.end}`}</span></div>)}</div>}
            </div>
            <div className="mt-6 flex items-center justify-end gap-3">
              <button type="button" onClick={() => setAvailabilityCleaner(null)} className="rounded-full border border-[#E5E7EB] px-4 py-2 text-sm font-semibold text-[#6B7280]">{ui.close}</button>
              <button type="button" onClick={() => startBooking(availabilityCleaner)} disabled={Boolean(user) && isCleaner()} className="inline-flex min-w-[120px] items-center justify-center rounded-full bg-[#4FC3F7] px-5 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60">{ui.reserve}</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {bookingCleaner ? (
        <div className="fixed inset-0 z-[80] flex items-center justify-center overflow-y-auto overscroll-contain bg-black/45 p-4" onClick={() => setBookingCleaner(null)}>
          <div className="w-full max-w-lg max-h-[calc(100dvh-2rem)] overflow-y-auto overscroll-contain rounded-3xl bg-white shadow-[0_24px_70px_rgba(17,24,39,0.3)]" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-center justify-between gap-3 border-b border-[#F0F4F8] px-6 py-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <button type="button" onClick={() => setBookingCleaner(null)} className="flex h-9 w-9 items-center justify-center rounded-full border border-[#E5E7EB] text-[#9CA3AF] transition-colors hover:border-[#DC2626] hover:text-[#DC2626]"><X size={16} /></button>
              </div>
            </div>
            {bookingStep === 1 ? (
              <div className="p-6">
                ...
                <div className="mt-5 space-y-4">
                  <label className="block text-sm font-semibold text-[#1A1A2E]">
                    {ui.address}
                    <select value={selectedSpaceId} onChange={(event) => setSelectedSpaceId(event.target.value)} className="mt-2 w-full rounded-xl border border-[#E5E7EB] bg-white px-3 py-2 text-sm outline-none focus:border-[#4FC3F7]">
                      {spaces.map((space) => (
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <label className="block text-sm font-semibold text-[#1A1A2E]">
                      {ui.date}
                      <input type="date" min={minBookDate} value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} className="mt-2 w-full rounded-xl border border-[#E5E7EB] bg-white px-3 py-2 text-sm outline-none focus:border-[#4FC3F7]" />
                    </label>
                    <TimePickerField value={selectedTime} onChange={setSelectedTime} label={ui.time} />
                  </div>
                </div>
                <p className="mt-4 rounded-2xl bg-[rgba(251,191,36,0.12)] px-4 py-3 text-xs font-medium text-[#92400E]">{ui.bookingAdjustDisclaimer}</p>
                {spaces.length === 0 ? <div className="mt-4 rounded-2xl border border-dashed border-[#D1E7F7] bg-[#F8FCFF] px-4 py-4 text-sm text-[#4B5563]"><p>{ui.noSpace}</p><a href={addSpacePath} onClick={(event) => { event.preventDefault(); navigateTo('clientAddSpace'); }} className="mt-3 inline-flex rounded-full bg-[#4FC3F7] px-4 py-2 text-xs font-semibold text-white">{ui.addSpace}</a></div> : null}
                {bookingError ? <p className="mt-4 text-sm font-semibold text-[#B91C1C]">{bookingError}</p> : null}
                <div className="mt-5 flex justify-end">
                  <button type="button" onClick={() => setBookingStep(2)} disabled={spaces.length === 0 || !selectedDate || !selectedTime} className="rounded-2xl bg-[#4FC3F7] px-6 py-2.5 text-sm font-bold text-white shadow-[0_4px_12px_rgba(79,195,247,0.35)] transition-all hover:bg-[#38B2E8] disabled:cursor-not-allowed disabled:opacity-60">
                    {ui.continue}
                  </button>
                </div>
              </div>
            ...
            {bookingStep === 2 ? (
              <div className="p-6">
                ...
                <div className="mt-5 flex items-center gap-3">
                  <button type="button" onClick={() => setBookingStep(1)} className="flex-1 rounded-2xl border border-[#E5E7EB] py-2.5 text-sm font-semibold text-[#6B7280] transition-colors hover:border-[#9CA3AF]">{ui.back}</button>
                  <button type="button" onClick={() => void reserveNow()} disabled={reserving || spaces.length === 0 || !selectedDate || !selectedTime} className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-[#4FC3F7] py-2.5 text-sm font-bold text-white shadow-[0_4px_12px_rgba(79,195,247,0.35)] transition-all hover:bg-[#38B2E8] disabled:opacity-60">
