...
export function CleanerDashboardPage() {
  ...
  const derivedHomeZone = useMemo(() => deriveZoneFromAddress(homeAddress), [homeAddress]);
  ...
  useEffect(() => {
    ...
    const loadCleanerProfile = async () => {
      ...
      setIsAddressDropdownOpen(false);
      setHomeAddressValidationError(null);
      const normalizedHomeZone = deriveZoneFromAddress(normalized.homeAddress);
  ...
  const handleSelectHomeAddress = (suggestion: AddressSuggestion) => {
    const derivedZone = deriveZoneFromAddress(suggestion.address);
    const derivedArea = derivedZone ? getZoneArea(derivedZone) : null;
    setHomeAddress(suggestion.address);
    setHomeAddressInput(suggestion.address.formatted);
    setDraftHomeZone(derivedZone ?? firstZoneName);
    setDraftHomeArea(derivedArea);
    setHomeArea(derivedArea);
    setAddressSuggestions([]);
    setIsAddressDropdownOpen(false);
    setHomeAddressValidationError(null);
  };
  ...
  const openAreaWizard = () => {
    ...
    const initialZone = initialHomeArea?.zone ?? draftHomeZone ?? firstZoneName;
    setDraftHomeZone(initialZone);
    setDraftHomeArea(initialHomeArea ?? getZoneArea(initialZone));
    setDraftServiceZone(serviceAreas[0]?.zone ?? initialZone);
    setServiceMode(null);
    setServiceModeError(false);
    setDraftServiceAreas(serviceAreas);
    setAreaSearch('');
    setAreaStep(initialHomeArea ? 3 : 1);
  ...
  const handleSave = async () => {
    ...
    const resolvedHomeArea = derivedHomeArea ?? homeArea;
    if (!resolvedHomeArea) {
